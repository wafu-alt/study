...
- ECMAScript는 클래스 정의에 대한 구문을 포함하지만 ECMAScript 객체는 C++, Smalltalk 또는 Java와 같이 기본적으로 클래스 기반은 아니다. 대신 오브젝트는 리터럴 표기법이나 오브젝트를 만든 다음 초기값을 속성에 할당하여 오브젝트의 전부 또는 일부를 초기화하는 코드를 실행하는 생성자(constructors)를 통해 다양한 방법으로 생성될 수 있다.
